...
class Database{
    ...
    public function register($name, $pass, $szam, $email) {
        //$password = password_hash($pass, PASSWORD_BCRYPT);
        $stmt = $this->db->prepare('INSERT INTO `users`(`name`, `password`,`szam`, `email`) VALUES(?,?);') ;
        $stmt->bind_param("ss", $name, $pass, $szam, $email);
        if($stmt->execute()){
            $_SESSION['login'] = true;
